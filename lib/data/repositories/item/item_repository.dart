...
class ItemRepository {
  ...
  Future<ItemModel> createItem(
    ...
    try {
      ...
      if (swipImage != null) {
        MultipartFile swip = await MultipartFile.fromFile(
          swipImage.path,
          filename: path.basename(swipImage.path),
        );
        parameters['swip_image'] = swip;
      }
  ...
  Future<ItemModel> editItem(
    ...
    print('swipImage in repo: ${swipImage?.path}');
    if (swipImage != null) {
      try {
        MultipartFile swip = await MultipartFile.fromFile(swipImage.path, filename: path.basename(swipImage.path));
        parameters["swip_image"] = swip;
        print('swip_image added to parameters');
      } catch (e) {
        print('error creating multipart file: $e');
      }
    } else {
      print('swipImage is null in repo');
    }
    if (otherImages != null && otherImages.isNotEmpty) {
      List<Future<MultipartFile>> futures = otherImages.map((imageFile) {
        return MultipartFile.fromFile(
