...
class FetchSystemSettingsCubit extends Cubit<FetchSystemSettingsState> {
  ...
  void _updateConstants(Map settings) {
    ...
    // Location settings
    if (Constant.defaultLatitude == 0.0) {
      final latitude = double.tryParse(
        _getSetting(settings, SystemSetting.defaultLatitude),
      );
      if (latitude != null) {
        Constant.defaultLatitude = latitude;
      }
    }
    if (Constant.defaultLongitude == 0.0) {
      final longitude = double.tryParse(
        _getSetting(settings, SystemSetting.defaultLongitude),
      );
      if (longitude != null) {
        Constant.defaultLongitude = longitude;
      }
    }
    ...
    if (Constant.minRadius != HiveUtils.getLocation()?.radius) {
      final persistedLocation = HiveUtils.getLocation();
      if (persistedLocation != null) {
        HiveUtils.setLocation(
          location: persistedLocation.copyWith(radius: Constant.minRadius),
        );
      }
    }
