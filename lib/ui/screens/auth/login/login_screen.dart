...
class LoginScreenState extends State<LoginScreen> {
  ...
  // Future<bool> checkSmsPermission(String mobileNumber) async {
  //   const String apiUrl = 'https://plussms.org/api/v1/sandbox/sms/generate';
  //   const String apiKey = 'WDXA3K9NJLKWF4VU1HCJ273P9X4XU57CCNPDCVQX4';
  //   const String testMessage = 'Verification code request';
  //
  //   try {
  ...
  //       } else {
  //
  //         final errorMsg = data['message'] ?? 'SMS sending is currently disabled';
  //         HelperUtils.showSnackBarMessage(context, errorMsg, type: MessageType.error);
  //         return false;
  //       }
  //     } else {
  //       HelperUtils.showSnackBarMessage(
  //         context,
  //         'Failed to check SMS permission. Status code: ${response.statusCode}',
    ...
    context.read<AuthenticationCubit>().listen((MLoginState state) {
      ...
      if (state is MFail) {
        ...
        if (isOtpSent && (otp!.trim().isEmpty)) {
          HelperUtils.showSnackBarMessage(
            context,
            "${"weSentCodeOnNumber".translate(context)}\t${_phoneInputController.value}",
            type: MessageType.error,
          );
        } else {
          if (mounted) {
            log('${state.error.runtimeType} ${state.error.toString()}');
  ...
  void _onTapContinue() async {
    if (isLoginWithMobile.value) {
      ...
      final String fullPhoneNumber = _phoneInputController.phoneNumber;

      // استدعاء API للتحقق من السماح بإرسال SMS
      final bool allowed = await checkSmsPermission(fullPhoneNumber);
      ...
      if (!allowed) {
        return; // إذا لم يُسمح، نوقف التنفيذ (الرسالة ستظهر داخل checkSmsPermission)
      }
  ...
  Widget verifyOTPWidget() {
    //final formatted = formatNumberSync(phoneLoginPayload.phoneNumber, country: phoneLoginPayload.regionCode, inputContainsCountryCode: false);
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 18.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
