...
class AdDetailsScreenState extends CloudState<AdDetailsScreen> {
  ...
  // --- إعدادات الدفع المسترجعة من API ---
  ...
  // جلب إعدادات الدفع (Stripe و BankTransfer)
  Future<void> _fetchPaymentSettings() async {
    try {
      final response = await Api.get(url: 'get-payment-settings');
      if (response['error'] == false) {
        _paymentSettings = response['data'];

        if (_paymentSettings?['Stripe'] != null && _paymentSettings!['Stripe']['status'] == 1) {
          _stripePublishableKey = _paymentSettings!['Stripe']['api_key'];
  ...
  Future<void> _fetchPlatformPrice() async {
    try {
      ...
      platformPrice = double.tryParse(settings['platform_price']?.toString() ?? '2');
    } catch (e) {
      platformPrice = 2.0;
    }
  }
  ...
  Future<void> _showPaymentDialog() async {
    ...
    if (_paymentSettings?['Stripe'] != null && _paymentSettings!['Stripe']['status'] == 1) {
      paymentOptions.add(
        ListTile(
          leading: Icon(Icons.credit_card, color: context.color.territoryColor),
          title: CustomText('Pay with Credit Card (Stripe)'),
          onTap: () {
            Navigator.pop(context); // إغلاق حوار الاختيار
    ...
    if (_paymentSettings?['bankTransfer'] != null && _paymentSettings!['bankTransfer']['status'] == '1') {
      final bank = _paymentSettings!['bankTransfer'];
      paymentOptions.add(
        ListTile(
          leading: Icon(Icons.account_balance, color: context.color.territoryColor),
          title: CustomText('Bank Transfer'),
          subtitle: CustomText('${bank['bank_name']} - ${bank['account_number']}'),
          onTap: () {
    ...
    if (paymentOptions.isEmpty) {
      HelperUtils.showSnackBarMessage(context, 'No payment methods available.');
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: CustomText('Choose Payment Method'.translate(context)),
        content: Column(
        ...
        actions: [
          TextButton(
            ...
            child: CustomText('Cancel'.translate(context)),
          ),
        ],
      ),
    );
  ...
  Future<void> _showBankTransferDetails() async {
    final bank = _paymentSettings?['bankTransfer'];
    if (bank == null) return;
    ...
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: CustomText('Bank Transfer Instructions'.translate(context)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            CustomText('Amount: ${platformPrice?.toStringAsFixed(2) ?? '2'} USD'),
            const SizedBox(height: 10),
            CustomText('Bank Name: ${bank['bank_name']}'),
            CustomText('Account Name: ${bank['account_holder_name']}'),
            CustomText('Account Number: ${bank['account_number']}'),
            if (bank['ifsc_swift_code'] != null)
              CustomText('Swift Code: ${bank['ifsc_swift_code']}'),
          ],
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: CustomText('Close'.translate(context)),
          ),
          ElevatedButton(
            onPressed: () {
              Navigator.pop(ctx);
            ...
            child: CustomText('I have transferred'.translate(context)),
          ),
        ],
      ),
    ).then((_) => _isProcessingPayment = false);
  }
  ...
  Future<void> _processStripePayment() async {
    ...
    try {
      ...
      final clientSecret = response['data']['payment_intent']['payment_gateway_response']['client_secret'];


      await Stripe.instance.initPaymentSheet(
        paymentSheetParameters: SetupPaymentSheetParameters(
          paymentIntentClientSecret: clientSecret,
          merchantDisplayName: 'get sparely',
      ...
      HelperUtils.showSnackBarMessage(
        context,
        'Payment successful! You can now contact the seller.'.translate(context),
      );
    } catch (e) {
      LoadingWidgets.hideLoader(context);
      ...
      HelperUtils.showSnackBarMessage(
        context,
        'Payment failed: $e'.translate(context),
      );
    } finally {
      _isProcessingPayment = false;
    }
  ...
  Future<void> _processBankTransferPayment() async {
    ...
    try {
      ...
      if (response['error'] == false) {
        HelperUtils.showSnackBarMessage(
          context,
          'Payment request sent successfully. Activation will occur once the transfer is confirmed.'.translate(context),
        );
      } else {
        throw Exception(response['message']);
      }
      ...
      LoadingWidgets.hideLoader(context);
      HelperUtils.showSnackBarMessage(context, 'Request failed: $e'.translate(context));
    } finally {
      _isProcessingPayment = false;
    }
  }
  ...
  void _showPhoneNumber() {
    final phoneNumber = model.contact ?? model.user?.mobile;
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: CustomText('Phone Number'.translate(context)),
        content: CustomText(phoneNumber),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            child: CustomText('Call'.translate(context)),
          ),
        ],
      ),
    );
  ...
  // دالة فتح الدردشة - كاملة مع شروط الفئات
  void _openChat() async {
    ...
    final chatedUser = context.read<GetBuyerChatListCubit>().getOfferForItem(model.id!);
        ...
        MaterialPageRoute(
          builder: (context) => MultiBlocProvider(
            ...
            child: ChatScreen(
              ...
              from: "item",
              isPurchased: (model.isPurchased == true) ? 1 : 0,
      ...
      return Row(
        ...
        children: [
          ...
          if (!isSwip)
            Expanded(
              child: _buildButton(
                isDonation ? "Contact Number".translate(context) : "Show Number".translate(context),
                    () {
                  if (isDonation) {
