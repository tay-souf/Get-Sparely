    ...
    return Padding(
      ...
      child: BlocBuilder<SliderCubit, SliderState>(
        builder: (context, SliderState state) {
          if (state is SliderFetchFailure) return const SizedBox.shrink();
          if (state is SliderFetchSuccess) {
            if (state.sliderlist.isEmpty) return const SizedBox.shrink();
      ...
      if (homeSlider.model!.subCategoriesCount! > 0) {
        ...
        Navigator.pushNamed(
          ...
          arguments: CategoryMetaData(
            categoryId: homeSlider.modelId.toString(),
            categoryIds: [homeSlider.modelId.toString()],
            title: homeSlider.model?.name?.localized ?? '',
          ),
        );
      }
    } else {
