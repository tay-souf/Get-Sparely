    ...
    return MaterialPageRoute(
      builder: (_) => BlocProvider(
        ...
        child: FilterScreen(
          ...
          showCategoryDropdown: arguments?['showCategoryDropdown'] ?? true,
        ),
      ),
    );
  }
...
class FilterScreenState extends State<FilterScreen> {
  late CategoryModel? selectedCategory = widget.filter?.category;

  late final TextEditingController minController = TextEditingController(
    text: widget.filter?.minPrice,
  );
  late final TextEditingController maxController = TextEditingController(
    text: widget.filter?.maxPrice,
  );

  late LeafLocation? location = widget.filter?.location;
  List<CustomFieldBuilder> moreDetailDynamicFields = [];

  late String postedOn =
      widget.filter?.postedSince ?? Constant.postedSince[0].value;

  late List<CategoryModel> categoryList = widget.categoryList ?? [];

  final ValueNotifier<bool> _isValidBudget = ValueNotifier(true);
  @override
  void dispose() {
    minController.dispose();
    maxController.dispose();
    _isValidBudget.dispose();
    super.dispose();
  }
  ...
  void setDefaultVal({bool isRefresh = true}) {
    if (isRefresh) {
      ...
      location = LeafLocation.global();
      selectedCategoryName = "";
      selectedCategory = widget.showCategoryDropdown
          ? null
    ...
    return Column(
      ...
      children: [
            ...
            children: <Widget>[
                ...
                child: budgetTextField(
                  ...
                  onChanged: (value) {
                    if (value.trim().isEmpty &&
                        searchBody.containsKey(Api.minPrice)) {
              ...
              Expanded(
                child: budgetTextField(
                  ...
                  onChanged: (value) {
                    if (value.trim().isEmpty &&
                        searchBody.containsKey(Api.maxPrice)) {
        ...
        ValueListenableBuilder(
          valueListenable: _isValidBudget,
