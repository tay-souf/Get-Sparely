...
class LocalAwesomeNotification {
  ...
  Future<void> createNotification({
    ...
    try {
      ...
      bool hasImage =
          notificationData.data["image"] != null &&
          notificationData.data["image"].toString().isNotEmpty;

      if (isChat) {
        int chatId =
            int.parse(notificationData.data['sender_id']) +
...
class NotificationController {
  ...
  static Future<void> onActionReceivedMethod(
    ...
    print(payload);
    print('${payload?['item_id']} ${payload?['item_id'].runtimeType}');
    if (payload?['type'] == Constant.notificationTypeChat) {
                ...
                child: Builder(
                  builder: (context) {
                    return ChatScreen(
                      ...
                      itemOfferPrice: NotificationService.getOfferPrice(
                        itemOfferPrice,
                      ),
        ...
        Constant.navigatorKey.currentContext!
            ...
            .addOrUpdateChat(
              ChatUser(
                ...
                amount: NotificationService.getOfferPrice(itemOfferPrice),
                buyerId: int.parse(HiveUtils.getUserId()!),
                createdAt: date,
                id: int.parse(itemOfferId!),
                sellerId: int.parse(senderId!),
                updatedAt: date,
                buyer: Buyer(
                  id: int.parse(HiveUtils.getUserId()!),
                  name: HiveUtils.getUserDetails().name,
                  profile: HiveUtils.getUserDetails().profile,
                ),
                item: Item(
                  id: int.parse(itemId),
                  name: itemName != null
                      ? LocalizedString(canonical: itemName)
                      : null,
                  image: itemImage,
                  price: double.parse(NotificationService.getPrice(itemPrice)!),
                  review: null,
                  isPurchased: 0,
      ...
      } else if (payload?['item_id'] != null && payload?['item_id'] != '') {
        var id = int.tryParse(payload?["item_id"] ?? "");
        if (id == null) return;
        Navigator.pushNamed(
          Constant.navigatorKey.currentContext!,
          Routes.adDetailsScreen,
    ...
    } else if (payload?['type'] == Constant.notificationTypeItemReview) {
      MainActivity.globalKey.currentState?.onItemTapped(3);
      HelperUtils.goToNextPage(
