...
class UiUtils {
  ...
  static Widget getImage(
    ...
    final scale = 2.0;
    // Try to give the cache a realistic target size so we avoid
    // decoding huge full‑resolution images for small grid cards.
  ...
  static SystemUiOverlayStyle getSystemUiOverlayStyle({
    ...
    bool isDarkMode = context.read<AppThemeCubit>().isDarkMode();
  ...
  static String monthYearDate(String date) {
    ...
    final supportsLocale = DateFormat.localeExists(Constant.currentLocale);

    return DateFormat(
      'MMMM yyyy',
      supportsLocale ? Constant.currentLocale : Intl.defaultLocale,
    ).format(dateTime);
  }
  ...
  static Future<Country> getSimCountry() async {
    ...
    Country simCountry = countryList.firstWhere(
      ...
      orElse: () {
        return countryList
            .where((element) => element.phoneCode == Constant.defaultPhoneCode)
            .first;
      },
    );
